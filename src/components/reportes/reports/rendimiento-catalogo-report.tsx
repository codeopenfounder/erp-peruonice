...
import { useReporteRendimientoCatalogo } from "@/hooks/queries/use-reportes"
...
const COLUMNS = [
  ...
  { header: "Unidades", key: "unidades_vendidas", width: 10, format: (v: unknown) => Number(v || 0).toFixed(2) },
  { header: "Costo Unit. (S/)", key: "costo_unitario", width: 14, format: (v: unknown) => Number(v || 0).toFixed(2) },
  { header: "Precio Venta (S/)", key: "precio_venta_unitario", width: 14, format: (v: unknown) => Number(v || 0).toFixed(2) },
  { header: "Ingreso (S/)", key: "ingreso_generado", width: 14, format: (v: unknown) => Number(v || 0).toFixed(2) },
  { header: "Costo (S/)", key: "costo_generado", width: 14, format: (v: unknown) => Number(v || 0).toFixed(2) },
  { header: "Ganancia (S/)", key: "ganancia_neta", width: 14, format: (v: unknown) => Number(v || 0).toFixed(2) },
]

export function RendimientoCatalogoReport({ open, onOpenChange }: Props) {
  const now = new Date()
  ...
  const { data, isLoading, isFetching } = useReporteRendimientoCatalogo(filters, generated && open)
  ...
  const handleExport = async (fmt: "xlsx" | "csv") => {
    ...
    try {
      await exportFlatData({
        filename: `rendimiento-catalogo-${dateFrom}-a-${dateTo}`,
        sheetName: "Catalogo",
        reportTitle: "Rendimiento de Catalogo y Mix de Ventas",
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="max-w-6xl max-h-[90vh] flex flex-col gap-0 p-0">
        <DialogHeader className="px-6 pt-6 pb-4 border-b border-border/50">
          <DialogTitle className="text-lg">Rendimiento de Catalogo</DialogTitle>
          <DialogDescription>
            Detalle de cada producto vendido con costo, ingreso y ganancia.
          </DialogDescription>
        </DialogHeader>
