...
import { useReporteTraficoVsEntradas } from "@/hooks/queries/use-reportes"
...
const COLUMNS = [
  ...
  { header: "Entradas Vendidas", key: "entradas_vendidas", width: 18, format: (v: unknown) => Number(v || 0) },
  { header: "QRs Escaneados", key: "qrs_escaneados", width: 18, format: (v: unknown) => Number(v || 0) },
  { header: "Diferencia (No Asistieron)", key: "diferencia", width: 24, format: (v: unknown) => Number(v || 0) },
]

export function TraficoVsEntradasReport({ open, onOpenChange }: Props) {
  const now = new Date()
  ...
  const { data, isLoading, isFetching } = useReporteTraficoVsEntradas(filters, generated && open)
  ...
  const handleExport = async (fmt: "xlsx" | "csv") => {
    ...
    try {
      await exportFlatData({
        filename: `trafico-vs-entradas-${dateFrom}-a-${dateTo}`,
        sheetName: "Trafico",
        reportTitle: "Trafico de Asistentes vs Entradas Vendidas",
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="max-w-6xl max-h-[90vh] flex flex-col gap-0 p-0">
        <DialogHeader className="px-6 pt-6 pb-4 border-b border-border/50">
          <DialogTitle className="text-lg">Trafico vs Entradas Vendidas</DialogTitle>
          <DialogDescription>
            Cruza entradas vendidas en el POS con QRs escaneados en pista para medir conversion y asistencia real.
          </DialogDescription>
        </DialogHeader>
